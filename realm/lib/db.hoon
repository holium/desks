...
/-  vstore=visas
/+  spaces-chat, passport-lib=passport, crypto-helper, bedrock-json
|%
::
:: helpers
::
...
::
++  row-to-json-path
  :: /[json-ified row object as string]
  |=  [=row =schema]
  ^-  path
  =/  jon=json
  (en-row:enjs row (~(put by *schemas) type.row schema))
  =/  as-str=@t  (en:json:html jon)
  /(scot %t as-str)/noun :: scot to encode safely
::
++  meets-constraints-edit
  ...
  ?~  tbl
    :: there's nothing in this table, so any row we add is unique along all possible columns
    :: so just do checks
    (meets-checks row u.const state bowl)
...
::
++  meets-checks
  |=  [=row =constraint state=state-2 =bowl:gall]
  ^-  ?
  =/  sch=schema
...
++  dejs
  ...
  ++  action
    ...
    ++  create-path
      ...
      =/  uconst    (~(get by p.jon) 'constraints')
      =/  const
        ?~  uconst  ~
        (de-constraints u.uconst)
    ...
    ::  "constraints": {
    ::    "/chat/0v0":{
    ::  // can specify either "uniques" or "checks" or both
    ::      "uniques": [
    ::        [1, 2, "ship.id"], // the combination of column 1, column 2 and the creating ship must be unique within that path on the /chat/0v0 table
    ::        [0] // just column zero must be unique
    ::      ],
    ::      "checks": [
    ::        {"agent": "my-agent", "scry": "/my/check/path"}
    ::      ]
    ::    }
    ::  }
      |=  jon=json
      ...
      =/  kvs
        ...
        =/  typ=type:common  (de-type s+k)
        [typ (de-constraint (~(got by p.jon) k) typ)]
      (~(gas by *constraints) kvs)
    ::
    ++  de-constraint
      |=  [jon=json =type:common]
      ^-  constraint
      ?>  ?=([%o *] jon)
      =/  uuniqs    (~(get by p.jon) 'uniques')
      =/  uniqs
        ?~  uuniqs  ~
    ...
    ::
    ++  de-type  de-type:dejs:bedrock-json
    ::
    ++  de-id  de-id:dejs:bedrock-json
    ::
    ++  de-space-path
      %+  cu
        path-to-space-path
